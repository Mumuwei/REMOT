...
class Vid2VidModelG(BaseModel):
    ...
    def initialize(self, opt):
        ...
        #定义生成器
        #12 9 12 24 64 3 batch [1]
        #print("yyy input_nc_T_1, input_nc_S_1, opt.label_nc_1, prev_output_nc, opt.ngf,opt.n_downsample_G, opt.norm, self.gpu_ids",input_nc_T_1, input_nc_S_1, opt.label_nc_1, prev_output_nc, opt.ngf,opt.n_downsample_G, opt.norm, self.gpu_ids)
        ...
        print('----------stage1G Networks initialized -------------') 
        print('-----------------------------------------------')

        # load networks
        if not self.isTrain or opt.continue_train or opt.load_pretrain_1:
            print("load pretrain model1G",opt.load_pretrain_1)
            self.load_network(self.netG, 'Parser', opt.which_epoch_1, opt.load_pretrain_1)
    ...
    def forward(self, input_TParsing, input_SPose, input_SParsing, input_SFG, fake_slo_prev, dummy_bs=0):
        ...
        #torch.Size([4, 1, 12, 256, 192]) torch.Size([4, 8, 3, 256, 192]) torch.Size([4, 8, 12, 256, 192]) torch.Size([4, 8, 3, 256, 192])
        #print("real_input_T, real_input_S, real_slo, real_sfg",real_input_T.shape, real_input_S.shape, real_slo.shape, real_sfg.shape)
        ...
        #print("result",fake_slo.shape)#b,8,12,256,192
        fake_slo_prev = fake_slo[:, -tG+1:].detach()
        fake_slo = fake_slo[:, tG-1:]
        
        
        #print("fake_slo_prev,fake_slo",fake_slo_prev.shape,fake_slo.shape)#b,2,12,256,192   b,6,12,256,192
#torch.Size([4, 6, 12, 256, 192]) torch.Size([4, 6, 12, 256, 192]) torch.Size([4, 6, 12, 256, 192]) torch.Size([4, 6, 12, 256, 192]) torch.Size([4, 6, 2, 256, 192]) torch.Size([4, 6, 1, 256, 192]) torch.Size([4, 1, 12, 256, 192]) torch.Size([4, 6, 3, 256, 192]) torch.Size([4, 7, 12, 256, 192]) torch.Size([4, 7, 3, 256, 192]) torch.Size([4, 2, 12, 256, 192])
        #print("fake_slo, fake_slo_raw, fake_slo_ls, fake_slo_raw_ls, flow, weight, real_input_T, real_input_S[:,tG-1:], real_slo[:,tG-2:], real_sfg[:,tG-2:], fake_slo_prev",fake_slo.shape, fake_slo_raw.shape, fake_slo_ls.shape, fake_slo_raw_ls.shape, flow.shape, weight.shape, real_input_T.shape, real_input_S[:,tG-1:].shape, real_slo[:,tG-2:].shape, real_sfg[:,tG-2:].shape, fake_slo_prev.shape)
    ...
    def generate_frame_train(self, netG, real_input_T, real_input_S, fake_slo_prev, start_gpu, is_first_frame):        
        ...
        #print("tG,n_frames_load",tG,n_frames_load)#3，6

        ### generate inputs              
        fake_slos_raw, fake_slos_ls, fake_slos_raw_ls, flows, weights = None, None, None, None, None
        ### sequentially generate each frame,依次生成每一帧
        for t in range(n_frames_load):
            ...
            #print("real_input_T_reshaped, real_input_S_reshaped, fake_slo_prevs_reshaped, use_raw_only",
                 #real_input_T_reshaped.shape, real_input_S_reshaped.shape, fake_slo_prevs_reshaped.shape, use_raw_only)
            #([4, 12, 256, 192]) torch.Size([4, 9, 256, 192]) torch.Size([4, 24, 256, 192])
