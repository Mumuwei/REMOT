...
#         netD_input_nc_1 = netD_input_nc_2 + opt.output_nc_3
        netD_input_nc_f = 1 + opt.output_nc_3
        netD_input_nc_1 = 15
        netD_input_nc_2 = 12
        ...
        print('----------stage3D Networks initialized -------------') 
        print('-----------------------------------------------')

        # load networks
        if opt.continue_train or opt.load_pretrain_3:          
            self.load_network(self.netD, 'D', opt.which_epoch_3, opt.load_pretrain_3)            
            self.load_network(self.netD_FG, 'D_FG', opt.which_epoch_3, opt.load_pretrain_3)            
            for s in range(opt.n_scales_temporal):
                self.load_network(getattr(self, 'netD_T'+str(s)), 'D_T'+str(s), opt.which_epoch_3, opt.load_pretrain_3)
            if opt.add_face_disc:
                self.load_network(self.netD_f, 'D_f', opt.which_epoch_3, opt.load_pretrain_3)
        ...
        self.loss_names = ['loss_G_SFG_L1','G_SI_VGG', 
                           'G_SI_GAN', 'G_SI_GAN_Feat',                         
                           'D_SI_real', 'D_SI_fake',
                           'G_SFG_GAN', 'G_SFG_GAN_Feat',
    ...
    def forward(self, scale_T, tensors_list, dummy_bs=0):
        ...
        #[real_SI, real_SFG, fake_SI, fake_SI_raw, fake_SFG, real_input_S, real_input_S_2, input_TParsing, real_input_TFG, real_input_BG, real_input_BG_flag, real_SI_prev, fake_SI_prev, real_SFG_full_prev, flow, weight, flow_ref, conf_ref]
        
        
        real_SI, real_SFG, fake_SI, fake_SI_raw, fake_SFG, real_input_S, real_input_S_2, input_TParsing, real_input_TFG, real_input_BG \
        , real_input_BG_flag, real_SI_prev, fake_SI_prev, real_SFG_full_prev, flow, weight, flow_ref, conf_ref = tensors_list
        
     
        #源和目标的人脸区域
        real_SLO_face = real_input_S[:,:,6:7]
        real_TLO_face = input_TParsing[:,:,3:4]
        
        self.bs = real_SI.size()[0]
        for b in range(self.bs):
            if not real_input_BG_flag[b]:
                real_SI[b] = real_SFG[b]
        ...
        real_SI, real_SFG, fake_SI, fake_SI_raw, fake_SFG, real_input_S, real_input_BG \
        , real_SI_prev, fake_SI_prev, flow, weight, flow_ref, conf_ref, real_SLO_face, real_TLO_face \
        = reshape([real_SI, real_SFG, fake_SI, fake_SI_raw, fake_SFG, real_input_S, real_input_BG \
                   , real_SI_prev, fake_SI_prev, flow, weight, flow_ref, conf_ref, real_SLO_face, real_TLO_face])
        ...
        #################### fake_B loss ####################   
        ### L1 loss
        loss_G_SFG_L1 = self.L1Loss(fake_SFG, real_SFG) * lambda_feat
        
        ### VGG + GAN loss 
        loss_G_SI_VGG = (self.criterionVGG(fake_SI, real_SI) * lambda_feat) if not self.opt.no_vgg else torch.zeros_like(loss_W)
        #GAN
        real_input_1 = torch.cat([real_input_S, real_input_BG], dim=1)#9,3,3
        ...
        #前景进行判别器
        loss_D_SFG_real, loss_D_SFG_fake, loss_G_SFG_GAN, loss_G_SFG_GAN_Feat = self.compute_loss_D(self.netD_FG, real_input_S, real_SFG, fake_SFG)

        
        #加入面部损失 
        if self.opt.add_face_disc:
            face_weight = 2
            ys_T, ye_T, xs_T, xe_T = self.get_face_region(real_TLO_face)
            ys_S, ye_S, xs_S, xe_S = self.get_face_region(real_SLO_face)
            if ys_T is not None and ys_S is not None:                
                
                real_input_f = real_SLO_face[:,:,ys_S:ye_S,xs_S:xe_S]
                
                loss_D_f_real, loss_D_f_fake, loss_G_f_GAN, loss_G_f_GAN_Feat = self.compute_loss_D(self.netD_f,
                    real_input_f, real_SFG[:,:,ys_S:ye_S,xs_S:xe_S], fake_SFG[:,:,ys_S:ye_S,xs_S:xe_S])  
        ...
        loss_list = [loss_G_SFG_L1,loss_G_SI_VGG, 
                     loss_G_SI_GAN, loss_G_SI_GAN_Feat,
                     loss_D_SI_real, loss_D_SI_fake, 
                     loss_G_SFG_GAN, loss_G_SFG_GAN_Feat,
    ...
    def get_losses(self, loss_dict, loss_dict_T, t_scales):
        ...
        loss_G = loss_dict['loss_G_SFG_L1'] + loss_dict['G_SI_GAN'] + loss_dict['G_SI_GAN_Feat'] + loss_dict['G_SI_VGG'] + loss_dict['G_SFG_GAN'] + loss_dict['G_SFG_GAN_Feat'] 
