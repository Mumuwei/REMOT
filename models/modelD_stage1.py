...
class Vid2VidModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # load networks
        if opt.continue_train or opt.load_pretrain_1:          
            self.load_network(self.netD, 'D', opt.which_epoch_1, opt.load_pretrain_1)            
            for s in range(opt.n_scales_temporal):
                self.load_network(getattr(self, 'netD_T'+str(s)), 'D_T'+str(s), opt.which_epoch_1, opt.load_pretrain_1)
