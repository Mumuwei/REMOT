...
# class for discriminator of layout GAN，stage1的判别器
class Vid2VidModelD(BaseModel):
    ...
    def initialize(self, opt):
        ...
        print('----------stage2D Networks initialized -------------') 
        print('-----------------------------------------------')

        # load networks,加载参数
        if opt.continue_train or opt.load_pretrain_2:          
            self.load_network(self.netD, 'D', opt.which_epoch_2, opt.load_pretrain_2)            
        ...
        #loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat, loss_G_L1
        self.loss_names = ['D_real', 'D_fake','G_GAN', 'G_GAN_Feat', 'G_L1','G_Vgg']                
        #self.loss_names_T = ['G_T_GAN', 'G_T_GAN_Feat', 'D_T_real', 'D_T_fake', 'G_T_Warp']     

        # initialize optimizers D                                           
    ...
    #开始计算loss
    #[real_slo, fake_slo, fake_slo_raw, fake_slo_ls, fake_slo_raw_ls, real_input_T, real_input_S, real_slo_prev, fake_slo_prev, flow, weight, flow_ref, conf_ref]
    def forward(self, scale_T, tensors_list, dummy_bs=0):
        ...
        if tensors_list[0][0].get_device() == self.gpu_ids[0]:
            #print("dummy_bs",dummy_bs)
            tensors_list = util.remove_dummy_from_tensor(tensors_list, dummy_bs)
            if tensors_list[0][0].size(0) == 0:                
        ...
        Fake_total_final,Real_total,Real_SP = tensors_list
        
        
        Fake_total_final,Real_total,Real_SP = reshape([Fake_total_final,Real_total,Real_SP])
        
        
        
        #print("Fake_total_final,Real_total,Real_SP ",Fake_total_final.shape,Real_total.shape,Real_SP.shape)

        _, _, self.height, self.width = Fake_total_final.size()
       


        #################### fake_B loss ####################        
        ###  GAN loss 
        loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat = self.compute_loss_D(self.netD,Real_SP,Real_total,Fake_total_final)
        #print("loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat",loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat)
        
        ### VGG + L1
        loss_G_VGG = (self.criterionVGG(Fake_total_final, Real_total) * lambda_feat) if not self.opt.no_vgg else torch.zeros_like(loss_W)
        loss_G_L1 = self.criterionL1(Real_total,Fake_total_final) * lambda_P
        #print("loss_G_L1",loss_G_L1)
       
        loss_list = [loss_D_real, loss_D_fake, loss_G_GAN, loss_G_GAN_Feat, loss_G_L1, loss_G_VGG]
        loss_list = [loss.view(-1, 1) for loss in loss_list]   
...
def reshape(tensors):
    ...
    fake_total = torch.cat([tensors[0][i] for i in range(5)],1)
    real_total = torch.cat([tensors[1][i] for i in range(5)],1)
    real_sp = torch.cat([tensors[2][i] for i in range(5)],1)
    
    
    _, _, ch, h, w = tensors[0][0].size()
    fake_total = fake_total.contiguous().view(-1, ch, h, w)
    real_total = real_total.contiguous().view(-1, ch, h, w)
    _, _, ch, h, w = tensors[2][0].size()
    real_sp = real_sp.contiguous().view(-1, ch, h, w)      
    
    return fake_total,real_total,real_sp